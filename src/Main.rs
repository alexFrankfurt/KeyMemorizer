...
fn history_folder() -> &'static str {
    HISTORY_FOLDER
        .get()
        .map(|s| s.as_str())
        .unwrap_or(r"D:\Dev\KeyMemorizer")
}

fn debug_enabled() -> bool {
    // Off by default. Enable via env var (KEYMEM_DEBUG=1/true) or by creating:
    // <history_folder>\debug.enabled
    let debug_file = format!("{}\\debug.enabled", history_folder());
    if fs::metadata(&debug_file).is_ok() {
        return true;
    }
...
fn debug_verbose() -> bool {
    ...
    let verbose_file = format!("{}\\debug.verbose", history_folder());
    fs::metadata(&verbose_file).is_ok()
}
...
fn main() {
    let folder_path = env::args()
        .nth(1)
        .unwrap_or_else(|| r"D:\Dev\KeyMemorizer".to_string());
    let _ = HISTORY_FOLDER.set(folder_path.clone());
    ...
    if debug_enabled() {
        ...
        if !debug_verbose() {
            log_debug(&format!("Tip: create {}\\debug.verbose for per-key debug", history_folder()));
        }
    }

    println!("Monitoring started. Saving to {}\\ai_history.log", history_folder());
...
fn log_char(text: &str) {
    let path = format!("{}\\ai_history.log", history_folder());
